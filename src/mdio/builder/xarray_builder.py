...
def to_xarray_dataset(mdio_ds: Dataset) -> xr_Dataset:  # noqa: PLR0912, PLR0915
    ...
    Raises:
        ValueError: If a variable requests Zarr v3 sharding while Zarr v2 is configured.
    """
    ...
    for v in mdio_ds.variables:
        ...
        if v.metadata is not None:
            metadata_dict = v.metadata.model_dump(exclude_none=True, mode="json", exclude={"chunk_grid", "shard_grid"})
        ...
        if shards is not None:
            if zarr_format == ZarrFormat.V2:
                msg = f"Variable {v.name!r} requests Zarr v3 sharding, but the configured store format is v2."
