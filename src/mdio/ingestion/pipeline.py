...
def run_segy_ingestion(  # noqa PLR0913
    ...
    # Update template chunk shape based on overrides
    if grid_overrides and grid_overrides.non_binned:
        # Re-map chunk sizes
        new_chunk_shape = []
        for dim in schema.dimensions:
            if dim.name == "trace":
                new_chunk_shape.append(grid_overrides.chunksize)
            elif dim.name == mdio_template.trace_domain:
                new_chunk_shape.append(mdio_template.full_chunk_shape[-1])
            else:
                idx = mdio_template.dimension_names.index(dim.name)
                new_chunk_shape.append(mdio_template.full_chunk_shape[idx])
        mdio_template._dim_names = tuple(d.name for d in schema.dimensions)
        mdio_template.full_chunk_shape = tuple(new_chunk_shape)
    elif grid_overrides and grid_overrides.has_duplicates:
        new_chunk_shape = list(mdio_template.full_chunk_shape)
        new_chunk_shape.insert(-1, 1)
        mdio_template._dim_names = tuple(d.name for d in schema.dimensions)
        mdio_template.full_chunk_shape = tuple(new_chunk_shape)
    ...
    strategy_registry = IndexStrategyRegistry()
    
    synthesize_dims = getattr(mdio_template, "synthesize_missing_dims", ())
    strategy = strategy_registry.create_strategy(grid_overrides, synthesize_dims)
